...
namespace ProfileGenerator
{
    public partial class FileUtils
    {
        ...
        {
            string? folderDir = Path.GetDirectoryName(folderName);
            if (string.IsNullOrEmpty(folderDir)) return false;
            var wildcard = new Wildcard(folderWildcard, caseSensitive);
            return (wildcard.IsMatch(folderDir));
        }

        public static string GetRunningFolder()
        {
#pragma warning disable IL3000 // Dereference of a possibly null reference.
            var folder = Assembly.GetExecutingAssembly().Location;
#pragma warning disable IL3000 // Dereference of a possibly null reference.
            if (string.IsNullOrEmpty(folder)) folder = System.AppContext.BaseDirectory;
